(() => {
  ...
  client.channel('room-directory').on('postgres_changes',{event:'*',schema:'public',table:'game_rooms'},refreshRooms).subscribe();refreshRooms();\n  $('#createRoom').onclick=()=>{const room=code();$('#roomCode').value=room;connect(room)};
