  ...
  function connect(room){currentRoom=room;
    ...
    channel.on('presence',{event:'sync'},renderPeople).on('broadcast',{event:'game-state'},({payload})=>window.dispatchEvent(new CustomEvent('qqe-remote-state',{detail:payload}))).subscribe(async status=>{
      if(status==='SUBSCRIBED'){
        ...
        window.qqeRoom={sendState:data=>channel.send({type:'broadcast',event:'game-state',payload:data}),role:$('#roomRole').value,playerName:$('#roomName').value.trim(),room,client};client.from('room_games').select('*').eq('room_code',room).maybeSingle().then(({data})=>{if(!data)client.from('room_games').insert({room_code:room,players:[],phase:'waiting'})});client.channel(`game-db:${room}`).on('postgres_changes',{event:'*',schema:'public',table:'room_games',filter:`room_code=eq.${room}`},payload=>window.dispatchEvent(new CustomEvent('qqe-game-record',{detail:payload.new}))).subscribe();
      }
    });
  }
